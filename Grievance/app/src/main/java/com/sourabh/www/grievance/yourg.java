...
public class yourg extends Fragment {

    public  static List<String> comp,status,id,user_id;

    ListView gd;
    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.activity_yourg, container, false);
        gd=(ListView) rootView.findViewById(R.id.grid);

        comp=new ArrayList<>();
        status=new ArrayList<>();
        id=new ArrayList<>();
        user_id= new ArrayList<>();
new yourg.logn().execute(String.valueOf(constants.id));
        return  rootView;
    }
    private class logn extends AsyncTask<String,String,String> {
            ...
            if (!result.equalsIgnoreCase("false") &&!result.equalsIgnoreCase("exception")&& ! result.equalsIgnoreCase("unsuccessful")) {
                ...
                try {
                    ...
                    for ( int i = 0; i < jsonArray.length(); i++) {
                        ...
                        user_id.add(jsonObject.getString("cust_id"));

                    }

                    CustomGridAdapter adapter=new CustomGridAdapter(getActivity(),comp,status,id,user_id);
        ...
        protected String doInBackground(String... params) {
            try {
                ...
                url = new URL("http://192.168.43.139/myg.php");
