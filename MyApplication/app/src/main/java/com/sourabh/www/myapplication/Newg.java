...
public class Newg extends Fragment {
   ...
   ListView lst;
    public  static List<String> comp,status;

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,  Bundle savedInstanceState) {
        View rootView = inflater.inflate(R.layout.activity_newg, container, false);

        lst=(ListView)rootView.findViewById(R.id.list) ;
        comp=new ArrayList<>();
        status=new ArrayList<>();
        new Newg.logn().execute(String.valueOf(constants.zone));
        return  rootView;
    }
    private class logn extends AsyncTask<String,String,String> {
            ...
            if (!result.equalsIgnoreCase("false") &&!result.equalsIgnoreCase("exception")&& ! result.equalsIgnoreCase("unsuccessful")) {
                ...
                try {
                    ...
                    for ( int i = 0; i < jsonArray.length(); i++) {
                        ...
                        comp.add(jsonObject.getString("area"));
                        status.add(jsonObject.getString("water"));

                    }

                    adapter adapter=new adapter(getActivity(),comp,status);
        ...
        protected String doInBackground(String... params) {
            try {
                ...
                url = new URL("http://192.168.43.139/newg.php");
